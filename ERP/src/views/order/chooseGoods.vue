                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-select
                            ...
                            :key="item.itemSkuId"
                            :label="item.skuNameCommon"
                            :value="item.itemSkuId">
                            </el-option>
                        </el-select>
...
export default {
    ...
    methods:{
        ...
        getTableData(){
            return oapi.getGoodsData(this.currUsersupplierId, this.tableParam).then((response) => {
        ...
        skuSelectChange(data, index){
            ...
            data.SKUSelectData.forEach((res) => {
                data.SKUSelectvalue.forEach((item) => {
                    if (res.itemSkuId == item) {
                        arr.push({
                            id: item,
                            totalPrice: res.totalPrice,
                            inventoryId: res.inventoryId,
                            itemNumber: res.itemNumber,
                            skuPropertyNames: res.skuNameCommon
                        })
                    }
                })
            })
        ...
        getPopoverData(data, index){
            ...
            if (!this.tableData[index].SKUSelectData) {
                this.$set(this.tableData[index], 'SKUSelectData', data.applyPriceDetailList)
            }
